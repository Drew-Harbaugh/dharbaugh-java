...
SELECT continent, round(CAST(AVG(lifeexpectancy)::numeric, 2) AS avg_expectancy --Postgres specific
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
ORDER BY avg_expectancy DESC;
...
SELECT DISTINCT headofstate FROM counrty;
